...
const SYSTEM_AGENTS = [
  ...
  },
  {
    ...
    model: "gemini-2.5-flash-lite",
    systemPrompt: "You are summarizing a completed AI voice call for someone who wasn't on it. Use BOTH the transcript AND the workflow questions already answered during the call. Never invent facts.\n\nReply with ONLY a valid JSON object — no markdown, no extra text — with these fields:\n{\n  \"summary\": \"2-3 sentence plain-English summary of what was discussed and the concrete outcome, using actual captured answers where relevant\",\n  \"keyPoints\": [\"one short bullet per concrete useful fact or decision\"],\n  \"outcome\": \"Interested | Not Interested | Callback Requested | No Answer | Wrong Number | Incomplete\",\n  \"callerName\": \"name actually stated by the caller, or null\",\n  \"callbackRequested\": true|false,\n  \"callbackRelativeMinutes\": number or null,\n  \"callbackLocalDateTime\": \"YYYY-MM-DDTHH:mm:ss or null\",\n  \"enquiryRequested\": true|false,\n  \"enquirySummary\": \"the specific unanswered question/request that needs human follow-up, or null\"\n}\n\nCALLBACK RULES: callbackRequested is true only when the caller actually asked for a later callback, including when the agent asks what time to call back and the caller supplies a time. A callback always wins over an enquiry. For a relative delay, put only the number of minutes in callbackRelativeMinutes. For a specific time/date, resolve it against {callerNow} and return YYYY-MM-DDTHH:mm:ss in callbackLocalDateTime. If no usable time was given, leave both timing fields null.\n\nENQUIRY RULES: enquiryRequested is true only when the caller asked a specific question/request that the agent genuinely could not answer or resolve. Do not mark questionnaire answers, ordinary objections, successful calls, or callback requests as enquiries. enquirySummary must contain the actual unresolved question/request.\n\nWorkflow answers captured during this call:\n{workflowAnswers}\n\nCaller local date/time:\n{callerNow}\n\nTranscript:\n{transcript}",
    tools: [],
