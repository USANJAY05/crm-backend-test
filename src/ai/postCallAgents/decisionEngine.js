...
function deriveTranscriptSignals(transcript) {
  ...
  const callerSuppliedTime = /\b(?:in\s+\d+\s*(?:minutes?|mins?|hours?|hrs?)|\d+\s*(?:minutes?|mins?|hours?|hrs?)\s+(?:later|from\s+now)|at\s+\d{1,2}(?::\d{2})?\s*(?:am|pm)?|\b\d{1,2}(?::\d{2})?\s*(?:am|pm)\b|(?:today|tomorrow)\s+at\s+\d{1,2}(?::\d{2})?\s*(?:am|pm)?)\b/i.test(callerText);
